...
export class Cursor extends Component<{
  ...
  #move(cursor: HTMLElement, char: HTMLElement) {
    ...
    // The marker span may now contain multiple characters (so that
    // connected scripts like Arabic keep shaping across the cursor).
    // Locate the precise glyph rect via a DOM Range over the cursor offset.
    const glyph = measureGlyph(char);
    const x = glyph.x;
...
function measureGlyph(char: HTMLElement): {
  x: number;
  w: number;
  text: string;
} {
  const text = char.firstChild;
  const offsetAttr = char.getAttribute("data-cursor-offset");
  if (text != null && text.nodeType === Node.TEXT_NODE && offsetAttr != null) {
    const node = text as Text;
    const offset = Math.min(Number(offsetAttr), node.length);
    const start = offset;
    ...
    // fall through to the offset-based fallback when the API is missing.
    if (typeof range.getBoundingClientRect === "function") {
      const rect = range.getBoundingClientRect();
      const parent = char.offsetParent as HTMLElement | null;
      const base =
        parent != null ? parent.getBoundingClientRect() : { left: 0 };
      return {
        x: Math.round(rect.left - base.left),
        w: Math.round(rect.width) || char.offsetWidth,
